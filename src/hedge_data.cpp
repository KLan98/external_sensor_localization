...
#define ROS_NODE_NAME "hedge_data"
...
external_sensor_localization::hedge_pos_ang hedge_pos_ang_msg;// hedge coordinates and angle message for publishing to ROS topic
external_sensor_localization::hedge_imu_fusion hedge_imu_fusion_msg;// IMU fusion data message for publishing to ROS topic
...
{uint8_t beaconReadIterations;
  ...
  ros::Publisher hedge_pos_ang_publisher = n.advertise<external_sensor_localization::hedge_pos_ang>(HEDGE_POSITION_WITH_ANGLE_TOPIC_NAME, 1000);  
  
  ros::Publisher hedge_imu_fusion_publisher = n.advertise<external_sensor_localization::hedge_imu_fusion>(HEDGE_IMU_FUSION_TOPIC_NAME, 1000);
  ...
  // ONLY PUBLISH TWO TOPICS hedge_pos_ang_msg AND hedge_imu_fusion_msg
  while (ros::ok())
  {
	...
	ROS_INFO("Address: %d, timestamp: %d, %d, X=%.3f  Y= %.3f  Z=%.3f" , // Angle: %.1f" , 	
